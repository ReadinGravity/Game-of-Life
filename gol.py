...
def drawGrid(ws=size):
    ...
    for i in range(count):
        canvas.create_line(0,i*ws,W,i*ws, fill="#cacccb")
    count=W//ws
    for i in range(count):
        canvas.create_line(i*ws,0,i*ws,H, fill="#cacccb")
...
def drawCells(oldmap,list_cell,ws = size):
    ...
    for y in range(height):
        for x in range(width):
            if oldmap[y][x]==1:
                list_cell.append(canvas.create_oval(x*ws,y*ws,(x+1)*ws,(y+1)*ws,fill='pink'))
